...
export default function ForgotPasswordAdmin() {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      const res = await fetch('http://localhost:3002/forgot_password_admin', {
        method: 'POST',
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      const res = await fetch('http://localhost:3002/reset_password_admin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
          ...
          {step === 1 ? (
            <form onSubmit={handleSendCode} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gray-800 text-white py-3 rounded-lg font-semibold shadow-lg hover:shadow-xl hover:bg-gray-900 transition-colors disabled:opacity-50"
              >
